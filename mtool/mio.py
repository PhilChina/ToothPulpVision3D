...
## 对按照 0 - 背景 1 - 器官 2 - 肿瘤 标记的标签文件进行提取
## 器官的标注区域包括肿瘤
def getOrganAndTumorLabel(dire, organ_dire, tumor_dire):
    '''
    从label文件夹中分离出Organ与Tumor的label，应用于0,1,2标注的数据
    其中Organ是包含Tumor的
    :param dire:
    :return:
    '''
    ...
    os.makedirs(organ_dire, exist_ok=True)
    os.makedirs(tumor_dire, exist_ok=True)

    files = get_files_name(dire=dire)
    for file in tqdm(files):
        label, origin, spacing, direction, image_type = get_medical_image(os.path.join(dire, file))
        organ = (label > 0).astype(np.int)
        tumor = (label > 1).astype(np.int)
        save_medical_image(organ, origin, spacing, direction,
                           target_path=os.path.join(organ_dire, file),
                           type=sitk.sitkUInt8)
        save_medical_image(tumor, origin, spacing, direction,
                           target_path=os.path.join(tumor_dire, file),
                           type=sitk.sitkUInt8)
