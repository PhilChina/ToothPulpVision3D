...
from mtool.mutils.mutils import get_3d_rotate, get_3d_interpolation


## 对图像进行数据增强（旋转）
def get_augment_data(images, angles=None, shifts=None, scales=None, bias=100):
    '''
    对图像进行数据增强
    ...
    for image in images:
        ...
        if scales is not None:
            ...
            image = get_3d_interpolation(image, dst_shape=shape)
            image[image < 0] = 0
...
def test_get_augment_data():
    ...
    mict, _, _, _, _ = get_medical_image("../../data/matched/microCT-registration/rigid-microCT-without_bg/ye wei dong-47.nrrd")
    ...
    angles = get_augment_data(images, angles=(90, 90, 90), shifts=None, scales=None)
    ...
    save_medical_image(np.asarray(angles[2] > 100).astype(np.int), os.path.join(test_dire, 'angle-anno.nrrd'), o, s, d, t)

    angles = get_augment_data(images, angles=None, shifts=(50, 50, 50), scales=None)
    ...
    angles = get_augment_data(images, angles=None, shifts=None, scales=(0.2, 2))
