...
def get_label_center(mask):
    ...
    assert len(np.unique(mask)) == 2, 'input image is not a label in ./mtool/mutils.py !'
...
def get_binary_mask(array, mode='otsu', fore=None):
    ...
    if mode == 'otsu':
        ...
        ## 二值化
        binary = np.asarray(array > thresh).astype(np.int)
...
class Skeleton:
    ...
    def get_graph(self):
        from mtool.mdata.mgraph import Graph
    ...
    def get_key_graph(self, threshold):
        ...
        while True:
            ...
            if sk_graph.get_num_verts() == self.sk_graph.get_num_verts():
                break
